...
exports.updateQuestionBoard = async function (title, content, boardId, userId, type) {
	...
	try {
		...
		if (myBoard.length < 1) return errResponse(baseResponse.UPDATE_BOARD_DENIED);
...
exports.deleteBoard = async function (boardId, userId, type) {
	...
	try {
		...
		if (isQuestion[0].BOARD_TYPE !== type) return errResponse(baseResponse.WRONG_BOARD_PATH);
