...
exports.deleteCartItem = async function (userId, lectureId) {
	...
	try {
		...
		if (checkUserCart.length < 1) return errResponse(baseResponse.CHECK_ITEM_NOT_EXIST);
		const basketId = checkUserCart[0].BASKET_ID;
...
exports.postCartItem = async function (userId, lectureId) {
	...
	try {
		...
		const checkUserCart = await paymentDao.checkCartsItem(connection, userId, lectureId);
		if (checkUserCart.length === 1) return errResponse(baseResponse.ITEM_ALREADY_EXIST);

		const getUserBasketId = await paymentDao.selectUserBasketId(connection, userId);
		const basketId = getUserBasketId[0].BASKET_ID;
...
exports.deleteReceipt = async function (userId, userBuyId) {
	...
	try {
		...
		if (checkUserBuy.length < 1) return errResponse(baseResponse.RECEIPT_NOT_EXIST);
