...
exports.oauthCreateUser = async function (email, name) {
	try {
		...
		const userIdResult = await userDao.insertUserInfo(connection, insertUserInfoParams);
		// 회원 가입 확인 Row
		if (userIdResult.affectedRows === 0) return errResponse(baseResponse.SERVER_ERROR);
...
exports.loginUser = async function (userId, password) {
	...
	try {
		...
		const refreshTokenInsertResult = await userDao.insertRefreshToken(
			connection,
			refreshToken,
			userInfoRows[0].USER_ID,
		);
...
exports.jwtTokenIssuance = async function (token) {
	...
	if (tokenCheckRow.length < 1) return errResponse(baseResponse.USER_TOKEN_NOT_EXISTENCE);
	...
	return response(baseResponse.SUCCESS("토큰 발급 성공"), {
		userId,
		jwt: jwtToken,
	});
};
...
exports.createKakaoBody = async function (code) {
	const clientId = secretConfig.kakaoRestKey;
	const redirectUri = secretConfig.kakaoRedirectUri;
	const clientSecret = secretConfig.kakaoSecretKey;
...
exports.oauthSignIn = async function (email) {
	...
	try {
		...
		if (refreshTokenInsertResult.affectedRows === 0) return errResponse(baseResponse.UPDATE_REFRESH_TOKEN_FAIL);

		return response(baseResponse.SUCCESS, {
			userId: userInfoRows[0].USER_IO,
			jwt: token,
			refreshToken,
		});
	} catch (err) {
