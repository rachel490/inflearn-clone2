...
exports.postUsers = async function (req, res) {
	...
	// 형식 체크 (by 정규표현식)
	if (!regexEmail.test(email)) return res.send(errResponse(baseResponse.SIGNIN_EMAIL_ERROR_TYPE));

	if (password.length === 0) return res.send(errResponse(baseResponse.SIGNIN_PASSWORD_EMPTY));
	// 기타 등등 - 추가하기
	...
	try {
		...
		switch (cooperation) {
			case "kakao": {
				...
				const userRow = JSON.parse(getRes);

				const { email } = userRow.kakao_account;

				const { name } = userRow.kakao_account;

				// 빈 값 체크
				if (!email) return res.send(response(baseResponse.KAKAO_LOGIN_EMAIL_EMPTY));
		...
		return res.send(errResponse(baseResponse.KAKAO_LOGIN_FAIL, err));
	}
};
...
exports.editPhoneNumber = async function (req, res) {
	...
	if (!regPhone.test(phoneNumber)) return res.send(errResponse(baseResponse.USER_PHONE_NUMBER_TYPE_ERROR));
