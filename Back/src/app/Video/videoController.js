...
exports.getWatchedVideo = async function (req, res) {
	...
	if (!classId) return res.send(errResponse(baseResponse.CLASS_VIDEO_EMPTY));

	const userLectureRow = await lectureProvider.checkUserLecture(userId, lectureId);

	if (userLectureRow.length < 1) return res.send(errResponse(baseResponse.CHECK_USER_LECTURES_FAIL));
...
exports.patchWatchedVideo = async function (req, res) {
	...
	if (watchedVideoRow.length < 1) return res.send(errResponse(baseResponse.USER_WATCHED_HISTORY_NOT_EXIST));
